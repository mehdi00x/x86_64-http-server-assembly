...
mov rdi, r9
lea rsi, [rip + http_post_request]
mov rdx, 4096
mov rax, 0
syscall
mov r14, rax          #store the POST request sizee

# Parse POST request - find first space
xor rcx, rcx
find_space_loop:
	cmp BYTE PTR [http_post_request + rcx], ' '
	je start_copy
	inc rcx
	jmp find_space_loop
...
copy_loop:
	mov al, BYTE PTR [http_post_request + rcx]
	cmp al, ' '
	je finish_copy
...
xor rcx, rcx
find_headers_end:
	cmp DWORD PTR [http_post_request + rcx], 0x0A0D0A0D
...
.section .bss
http_post_request: .skip 4096
path_file: .skip 256
flag: .skip 256
data_length: .skip 256